...
// UTM 追踪参数
function buildShareUrl(baseUrl: string, source: string): string {
...
export default function SocialShare({
  ...
  const pageUrl = typeof window !== "undefined"
    ? `${window.location.origin}/images/${imageId}`
    : `/images/${imageId}`;

  // 复制链接
  const copyLink = useCallback(async () => {
    const url = buildShareUrl(pageUrl, "copy_link");
    try {
  ...
  const shareToWeibo = useCallback(() => {
    const url = buildShareUrl(pageUrl, "weibo");
  ...
  }, [pageUrl, imageTitle, imageUrl]);

  // 微信分享（显示二维码提示）
  const shareToWeChat = useCallback(() => {
    const url = buildShareUrl(pageUrl, "wechat");
  ...
  const shareToQQ = useCallback(() => {
    const url = buildShareUrl(pageUrl, "qq");
    const title = `${imageTitle} - ImageGallery 精选壁纸`;
  ...
  const shareToTwitter = useCallback(() => {
    const url = buildShareUrl(pageUrl, "twitter");
  ...
  const nativeShare = useCallback(async () => {
    const url = buildShareUrl(pageUrl, "native");
    if (navigator.share) {
      try {
  ...
  const platforms = [
    ...
    },
    {
      ...
      color: copied ? "text-green-500" : "text-[var(--color-ink)]",
      bgColor: copied ? "bg-green-50 dark:bg-green-900/10" : "bg-[var(--color-surface-card)]",
      onClick: copyLink,
    },
  ];
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-[var(--color-surface-soft)] rounded-t-2xl sm:rounded-2xl shadow-2xl max-w-sm w-full mx-0 sm:mx-4 overflow-hidden"
              ...
              {platforms.map((platform) => {
                ...
                return (
                  <button
                    ...
                    className="flex flex-col items-center gap-2 p-2 rounded-xl hover:bg-[var(--color-surface-card)] transition-colors"
                  >
                    <div className={`w-11 h-11 rounded-full ${platform.bgColor} flex items-center justify-center ${platform.color}`}>
                      <Icon />
                    </div>
                    <span className="text-[11px] text-[var(--color-mute)] truncate w-full text-center">
                      {platform.name}
                    </span>
                  </button>
                );
