...
export async function GET() {
  try {
    ...
    const users = await db
      .selectFrom("users")
      .select(["banner", "bio", "social_links", "featured_collections"])
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (social_links !== undefined) {
      ...
      updates.social_links = JSON.stringify(social_links);
    }
    ...
    if (featured_collections !== undefined) {
      ...
      if (featured_collections.length > 0) {
        const validCollections = await db
          ...
          .select("id")
          .where("id", "in", featured_collections)
      ...
      updates.featured_collections = JSON.stringify(featured_collections);
    }
